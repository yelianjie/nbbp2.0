  ...
  <div class="scroll-container scroll">
    <div class="detail-item flex flex-align-center bg2" v-for="(v, i) in list" :key="i">
      <div class="detail-item-left flex-1">
        <p class="overflow fff-bp f14">兑换牛角</p>
        <p class="f13 scolor bottom-line">{{v.create_time}}</p>
      </div>
      <div class="detail-item-right">
        <p class="overflow f14 ncolor">-{{v.exchange_money}}元</p>
        <p class="f13 scolor bottom-line">+{{v.exchange_money}}个牛角</p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    infiniteHandler ($state) {
      getExchangeList(this.params).then((res) => {
