...
<!--     <div class="detail-item flex flex-align-center fff" v-for="(v, i) in list" :key="i">
      <div class="detail-item-left flex-1">
        <p class="overflow f14">{{v.title}}×{{v.odr_num}}</p>
        <p class="f13 scolor bottom-line" >来自 {{v.nickname}} <template v-if=''>送给 {{v.to_nickname}}</template></p>
      </div>
      <div class="detail-item-right">
        <p class="overflow f14 ncolor">{{v.order_money}}元</p>
        <p class="f13 scolor bottom-line">{{v.create_time}}</p>
    ...
    <x-dialog  v-model="dialogVisible" hide-on-blur >
      <div class ='dialog' >
        <div class='dialog-title f18'>收益说明</div>
        <p>1、商户收益为您从每一笔订单分得的收益分成。</p>
